...
`;
  ...
  initAgentUI() {
    ...
    this.agents.forEach(agent => {
      ...
      item.className = 'agent-item' + (agent.id === this.activeAgentId ? ' active' : '');
      item.dataset.agentId = agent.id;
      item.innerHTML = `<span class="agent-icon">${agent.icon || ''}</span><span class="agent-name">${agent.name}</span>`;
      item.addEventListener('click', () => this.switchAgent(agent.id));
      agentList.appendChild(item);
    });

    // Filter chat list to active agent
    this.renderChatList();
  }
  ...
  switchAgent(agentId) {
    ...
    this.currentChatId = agentChats[0]?.id || null;
    this.renderChatList();
    this.renderMessages();
...
Example: [0, 2, 5]`;
  ...
  renderChatList() {
    ...
    const allChats = this.agents.length > 0
      ? Object.entries(this.chats).filter(([_, c]) => c.agentId === this.activeAgentId || (!c.agentId && this.activeAgentId === 'main'))
